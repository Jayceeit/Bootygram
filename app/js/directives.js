...
angular.module( 'myApp.directives', [ 'myApp.filters' ] )
      ...
      var update = function () {
        ...
        if ( peerID > 0 ) {
          ...
          element.html(
            icon_list +
            prefix +
            ( override[ key ] || user[ key ] || '' ).valueOf() + unescape( "%20%u202A" ) + username + ' ( ' + peerID + ' )' +
  ...
  <circle class="progress-arc-bar"></circle>\
    ...
    function updateProgress( bar, progress, fullLen ) {
      ...
      bar.css( {
        strokeDasharray: ( progress * fullLen ) + ', ' + ( ( 1 - progress ) * fullLen )
      } )
    }
    ...
    return {
      ...
      link: function ( $scope, element, attrs ) {
        ...
        if ( intermediate ) {
          ...
          bar.css( {
            stroke: 'url(#grad_intermediate' + curNum + ')'
          } )
        } else {
  ...
  .directive( 'myEmojiImage', function ( RichTextProcessor ) {
    function link( $scope, element, attrs ) {
      ...
      var html = RichTextProcessor.wrapRichText( emoji, {
        noLinks: true,
        noLinebreaks: true
      } )
